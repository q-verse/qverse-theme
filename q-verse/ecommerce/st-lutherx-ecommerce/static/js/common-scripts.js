...
function edlySetupWPFooter() {
    let edlyWPBaseURL = $('body').attr('data-wordpress-base-url');

    if (edlyWPBaseURL !== '' && edlyWPBaseURL !== '#' && typeof edlyWPBaseURL !== 'undefined') {
        let footerLoader = $('.wrapper-footer .loading');
        footerLoader.show();
        let edlyWPFooterURL = edlyWPBaseURL + '/wp-json/edly-wp-routes/footer';
        let request = new XMLHttpRequest();
        request.onreadystatechange = function() {
            if (this.readyState === 4 && this.status === 200) {
                $('.wrapper.wrapper-footer').html(JSON.parse(this.responseText));
            }
        };
